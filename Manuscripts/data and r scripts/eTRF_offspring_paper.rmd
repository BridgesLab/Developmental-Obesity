...
#preHFD FE
eTRF.M.NCD.ITT.data<-NCD.ITT.raw.data%>%filter(treatment=="eTRF"&sex=="male")%>%
  filter(time<60)%>%
  rename(y=glucose,
         t=time)%>%
  select(ID, t, y)

function.decay<-function(t,ampl,tau)(ampl*exp(-t/tau))
fit<-nls(y~ function.decay(t, myA,myT), data = eTRF.M.NCD.ITT.data, start=list(myA = 270, myT=15))

summary(fit)
fitY<-function.decay(eTRF.M.NCD.ITT.data$t, 205.43, 61.52)
fitY<-predict(fit)
 summary(fit, correlation = TRUE)#-0.67
require(nlme)
plot(residuals(fit))#suggests fit isn't great
#qqplot of the fit model
qqplot(eTRF.M.NCD.ITT.data$t,residuals(fit))#
#AL dataset
AL.M.NCD.ITT.data<-NCD.ITT.raw.data%>%filter(treatment=="AL"&sex=="male")%>%
  #filter(time<60)%>%
  rename(y=glucose,
         t=time)%>%
  select(ID, t, y)
#fit model
fit.2<-nls(y~ function.decay(t, myA,myT), data = AL.M.NCD.ITT.data, start=list(myA = 270, myT=15))
#analyze model for fit parameters
summary(fit.2)
 summary(fit.2, correlation = TRUE)#-0.71
fitY.2<-function.decay(AL.M.NCD.ITT.data$t, 187.75, 76.51)
fitY.2<-predict(fit.2)
#qqplot
qqplot(AL.M.NCD.ITT.data$t,residuals(fit.2))#
summary(fit.2, correlation = TRUE)#p=70.7
require(nlme)
plot(residuals(fit.2))#suggests fit isn't great

#comparison of nested models

plot(AL.M.NCD.ITT.data$t,AL.M.NCD.ITT.data$y)
```


```{r feed-eff}
Feed.Eff.Data<-MRI.raw.data%>%
  group_by(ID, sex, cage, treatment)%>%
  filter(assay=="Body Weight")%>% 
  pivot_wider(names_from = assay, values_from = gram.values)%>%
  as.numeric(`Body Weight`)%>%
 ...
 #as.numeric(unlist(Feed.Eff.Data$`Body Weight`))
keep<-c("1","8","20")
intake.eff.NCD<-intake.data.NCD%>%
  filter(week%in%keep)%>%
  select(-c(IDs, start, end, notes))
intake.eff.HFD<-intake.data.HFD%>%
   filter(week%in%keep)%>%
  select(-c(IDs, start, end, notes))


NCD.Feed.eff.data<-full_join( intake.eff.NCD, Feed.Eff.Data, by = c("week", "cohort", "cage","treatment","sex"))%>%
  na.omit()

Feed.data<-NCD.Feed.eff.data%>%
   group_by(ID)%>%
  mutate(kcals.gained.NCD = (`Body Weight`[week==8]-`Body Weight`[week==1]))


#,
         kcals.gained.HFD = (`Body Weight`[week==20]-`Body Weight`[week==8]))
```
#maternal counts
```{r maternal-info}
MRI.raw.data %>%
