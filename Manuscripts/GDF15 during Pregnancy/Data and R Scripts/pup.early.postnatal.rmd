...
avg.litter%>%
  ...
  scale_fill_manual(values = defense.color.scheme)+
  labs(title = "Total Litter Size", y = "average pups per dam")+
  theme_bw()+
   theme(legend.position = "none",text = element_text(size=14))
#stats
...
avg.litter%>%
  ...
  scale_fill_manual(values = defense.color.scheme)+
  labs(title = "Litter Size (living pups)", y = "average pups per dam")+
  theme_bw()+
  theme(legend.position = "none",text = element_text(size=14))
#stats
...
survival.data%>%
  ...
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se),width =0.3)+
  scale_fill_manual(values = defense.color.scheme)+
  labs(title = "Pup Survival to PND 3", y = "pups survived (% of born alive)")+
  theme_bw()+
  theme(legend.position = "none",text = element_text(size=14))
#stats
shapiro.test(mutated.litter.data$percent.survival[mutated.litter.data$Genotype=="-/-"])#not normal
shapiro.test(mutated.litter.data$percent.survival[mutated.litter.data$Genotype=="+/+"])#not normal
...
ggplot(avg.wt.data, aes(day, avg.wt))+
  ...
  theme_bw()+
  theme(legend.background = element_blank(),legend.position = c(0.15,0.85),text = element_text(size=14))
#stats
