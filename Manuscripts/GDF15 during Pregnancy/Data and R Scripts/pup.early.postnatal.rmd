...
pup.weight.data%>%
  filter(!(is.na(sex)))%>%
ggplot( aes(day, weight, col = Genotype, pch = sex))+
...
##
lmer(weight~ day +  (1|MouseID), data = full.sex.data)%>%summary
corr.effect<-lmer(weight~ day +  (day|MouseID), data = full.sex.data)%>%ranef#slope and intercept for each, presuming they are correlated
corr.effect$MouseID%>%
  ggplot(aes(x=`(Intercept)`, y=`day`))+
  geom_point()


lmer(weight~ day +  (day|MouseID), data = full.sex.data)%>%summary



non.cor.effect<-lmer(weight~ day +  (day||MouseID), data = full.sex.data)%>%ranef#group of pups have same slopes

non.cor.effect$MouseID%>%
  ggplot(aes(x=`(Intercept)`, y=`day`))+
  geom_point()
...
#do (day||UniquepupID)
#which model is best
anova(lmer(weight~ day +  (day||MouseID), data = full.sex.data),lmer(weight~ day +  (day|MouseID), data = full.sex.data))


anova(lmer(weight~ day + sex:day + sex + Genotype + Genotype:day + Genotype:sex:day+  (day||MouseID), data = full.sex.data),lmer(weight~ day + sex:day + sex  +  (day||MouseID), data = full.sex.data))
```