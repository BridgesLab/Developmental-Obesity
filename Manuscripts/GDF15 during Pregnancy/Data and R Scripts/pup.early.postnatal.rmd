...
ggplot(avg.wt.data, aes(day, avg.wt))+
  ...
  labs(title = "Offspring postnatal body weight", y = "body weight (grams)")
#stats
full.sex.data<-pup.weight.data%>%
  filter(!(is.na(sex)))
null.lme<-lmer(weight ~ 1 +  (1|MouseID) + (day||pup.number)+(1|pup.number), data = full.sex.data)

day.lme<-lmer(weight ~ day +  (1|MouseID) + (day||pup.number)+(1|pup.number), data = full.sex.data)
anova(null.lme, day.lme)
day.lme%>%summary
sex.lme<-lmer(weight ~ day + sex  +(1|MouseID) + (day||pup.number)+(1|pup.number), data = full.sex.data)
anova(sex.lme, day.lme)
sex.lme%>%summary
geno.lme<-lmer(weight ~ day + sex + Genotype +  (1|MouseID) + (day||pup.number)+(1|pup.number), data = full.sex.data)
