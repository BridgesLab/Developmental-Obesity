...
avg.itt.data%>%
  ...
  labs(title = "Effect of Pregnancy on Insulin Tolerance", y = "glucose (mg/dL)", x="Insulin (minutes)")+
...
AUC.data%>%
  ...
  labs(title = "Area Under the Curve", y="glucose (mg/dL)",x="Insulin (minutes)")+
  theme_bw()+
...
avg.itt.data%>%
  ...
  geom_line()+
  geom_errorbar(aes(ymax = glucose_mean + glucose_se, ymin = glucose_mean - glucose_se))+
  labs(title = "Effect of Dexamethasone on Insulin Tolerance", y = "glucose (mg/dL)", x="Insulin (minutes)")+
...
ggplot( aes(experiment.time, Weight, col = Treatment))+
  ...
  geom_smooth(method = loess,aes( col = Treatment))+
    ...
    scale_x_continuous(breaks=seq(-14, 35, 7))+
  theme_bw(base_size=18)+
