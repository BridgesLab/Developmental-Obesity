...
ITT.data%>%
  ...
  labs(title = "Fasting Blood Glucose", y = "mg/dL")
#stats
FBG.data <-ITT.data%>%
  filter(time == "0")
...
AUC.data%>%
  ...
  scale_fill_manual(values = color.scheme)+
  theme(legend.position = "none")+
  labs(title = "Area Under the Curve", y = "mg/dL")
...
t.test(AUC ~ Genotype, data = AUC.data,var.equal = FALSE)$p.value#0.7
```
...
ggplot(summary.models, aes(Genotype, mean, fill = Genotype))+
  ...
  geom_errorbar(aes(ymin = mean-se, ymax = mean + se), width = 0.3)+
  scale_fill_manual(values = color.scheme)+
  labs(title = "Rate of Drop",y="rate of glucose disposal")+
  theme(legend.position = "none")
 
#stats
t.test(slope ~ Genotype, data = models)
anova(drop.aov)
```
