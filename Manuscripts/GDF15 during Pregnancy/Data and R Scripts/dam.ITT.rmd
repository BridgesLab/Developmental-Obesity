...
```{r time-course}
 ITT.data%>%
  ...
  theme(legend.position = c(0.85, 0.85),text = element_text(size=14))+
  theme_bw()
...
ggplot(aes(time, mean, col = Genotype))+
  ...
  scale_color_manual(values = color.scheme)+
  geom_line(aes(col = Genotype))+
  labs(title = "Insulin tolerance test", y= "relative glucose (% baseline)",x="Time From Injection (minutes)")+
  theme_bw()+
...
FBG.data <-ITT.data%>%
  filter(time == "0")
shapiro.test(FBG.data$glucose[FBG.data$Genotype=="-/-"])#normal
shapiro.test(FBG.data$glucose[FBG.data$Genotype=="+/+"])#normal
...
AUC.data%>%
  ...
  scale_fill_manual(values = color.scheme)+
  labs(title = "Area Under the Curve", y = "glucose (mg/dL)/dam")+
  theme_bw()+
  theme(legend.position = "none",text = element_text(size=14))
#stats
shapiro.test(AUC.data$"AUC"[AUC.data$Genotype=="+/+"])#normal
shapiro.test(AUC.data$"AUC"[AUC.data$Genotype=="-/-"])#normal
...
ggplot(summary.models, aes(Genotype, mean, fill = Genotype))+
  ...
  geom_errorbar(aes(ymin = mean-se, ymax = mean + se), width = 0.3)+
  scale_fill_manual(values = color.scheme)+
  labs(title = "Initial Rate of Glucose Change",y="rate of glucose disposal")+
  theme_bw()+
  theme(legend.position = "none",text = element_text(size=14)) 
#stats
shapiro.test(models$slope[models$Genotype=="-/-"])#normal
shapiro.test(models$slope[models$Genotype=="+/+"])#normal
