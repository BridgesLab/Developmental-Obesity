...
#stats
null.rel <- lmer(rel.glucose ~ 1 + (1|MouseID), ITT.data)
time.rel <- lmer(rel.glucose ~ time + (1|MouseID), ITT.data)
anova(null.rel, time.rel)#significant effect of time
geno.rel<- lmer(rel.glucose ~ time + Genotype + (1|MouseID), ITT.data)
anova(time.rel, geno.rel)#p=0.19
geno.int.rel<-lmer(rel.glucose ~ time*Genotype + (1|MouseID), ITT.data)
anova(geno.rel, geno.int.rel)#p=0.12
Anova(geno.rel)
fixef(geno.rel)
```
