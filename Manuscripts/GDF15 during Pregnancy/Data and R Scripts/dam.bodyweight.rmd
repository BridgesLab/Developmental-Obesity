...
postnatal.bw.data<-bw.data%>%
  ...
  mutate(loss = last(gram.values)-first(gram.values),
         percent.loss = (loss/first(gram.values))*100)
#filter data for one observation per animal
postnatal.bw.data.cleaned<-postnatal.bw.data%>%
  group_by(MouseID, Genotype)%>%
  distinct(MouseID, .keep_all = T)%>%
  select(MouseID, Genotype, loss, percent.loss)
#bar plot, total loss
...
#bar plot, percent loss
postnatal.bw.data.cleaned%>%
   group_by(Genotype)%>%
  summarize(avg.percent = mean(percent.loss),
            error.percent = se(percent.loss))%>%
ggplot(aes(Genotype,avg.percent, fill = Genotype))+
  geom_col()+
  scale_fill_manual(values = color.scheme)+
  geom_errorbar(aes(ymin = avg.percent - error.percent, ymax = avg.percent + error.percent), width =0.3)+
  labs(title = "Postnatal weight loss", y = "Postnatal weight loss (%)")+
  theme_bw(base_size=18)+
  theme(legend.position = "none")

#stats, percent loss
shapiro.test(postnatal.bw.data.cleaned$percent.loss[postnatal.bw.data.cleaned$Genotype=="Gdf15 Knockout"])#p=0.4
shapiro.test(postnatal.bw.data.cleaned$percent.loss[postnatal.bw.data.cleaned$Genotype=="Wild-type"])#p=0.6
leveneTest(percent.loss ~ Genotype, postnatal.bw.data.cleaned)#p=0.053
t.test(percent.loss ~ Genotype, postnatal.bw.data.cleaned)$p.value#p=0.143
