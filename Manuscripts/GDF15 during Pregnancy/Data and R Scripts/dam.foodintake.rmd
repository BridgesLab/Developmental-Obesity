...
prenatal.final%>%
  ...
  labs(title = "Prenatal cumulative food intake", y="kcal/mouse")+
...
t.test(final.pre ~ Genotype, data = prenatal.final)#p=0.5
```
...
sub.post%>%
  ...
  geom_errorbar(aes(ymin = avg.kcal -error.kcal, ymax = avg.kcal + error.kcal),width=0.3)+
  scale_fill_manual(values = color.scheme)+
  labs(title = "Postnatal cumulative food intake", y="kcal/mouse")+
  theme(legend.position = "none")
...
t.test(postnatal.final ~ Genotype, sub.post)#0.9
