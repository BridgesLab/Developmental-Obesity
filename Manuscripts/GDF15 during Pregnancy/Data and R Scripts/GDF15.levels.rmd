...
test<-both.plate.data%>%
  ...
  geom_errorbar(data = test, aes(ymin = avg-error, ymax = avg+error))+
...
ggplot()+
  ...
  labs(title = "GDF15 in Dams",y="GDF15 (pg/mL)")+
  facet_grid(.~Genotype)+
  #geom_errorbar(summary.Gdf, aes(x=time, y=avg, ymax = avg+error, ymin = avg-error, group = Genotype))+
