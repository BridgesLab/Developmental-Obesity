...
ggplot(summary.models, aes(treatment, mean, fill = treatment))+
  ...
  scale_fill_manual(values = color.scheme)+
  labs(title = "Rate of Drop in Glucose",y="mg/dL per minute")+
   scale_fill_manual(values = color.scheme)+
  theme_bw()+
  theme(legend.position = "none",text = element_text(size=18))
...
ggplot(summary.rebound, aes(treatment, mean, fill = treatment))+
  ...
  geom_errorbar(aes(ymin = mean-se, ymax = mean + se), width = 0.3)+
  scale_fill_manual(values = color.scheme)+
  labs(title = "Rate of Glucose Increase after Hypoglycemia",y="delta glucose (mg/dL/min")+
   scale_fill_manual(values = color.scheme)+
  theme_bw()+
  theme(legend.position = "none",text = element_text(size=18))
...
ggplot(aes(time, avg.glu, col = treatment))+
  ...
  theme_bw()+
  labs(title = "Insulin Tolerance Test, E16", y = "Glucose (mg/dL)", x = "Time after injection (minutes)")+
  theme(legend.position = "none",text = element_text(size=18))

#linear mixed effect modeling
null.ITT<-lmer(glucose~ 1 + (1|ID), data = ITT.data)
time.ITT<-lmer(glucose~ time + (1|ID), data = ITT.data)
anova(null.ITT, time.ITT)#p=0.3
simple.group.ITT<-lmer(glucose~ time + treatment + (1|ID), data = ITT.data)
anova(time.ITT, simple.group.ITT)#0.04
int.group.ITT<-lmer(glucose~ time*treatment + (1|ID), data = ITT.data)
anova(simple.group.ITT, int.group.ITT)#significant
anova(int.group.ITT)%>%kable(caption = "glucose~ time*treatment + (1|ID)")

fixef(int.group.ITT)
summary(int.group.ITT)
```